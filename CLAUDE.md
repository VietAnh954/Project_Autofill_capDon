...
> File này được Claude Code đọc TỰ ĐỘNG mỗi phiên làm việc trong VSCode.
> KHÔNG xoá. KHÔNG sửa nếu chưa hiểu mục đích.

## Imports (Claude Code tự load các file này cùng CLAUDE.md)
...
LOOP:
  ...
  9. Commit git theo Conventional Commits (xem GIT_WORKFLOW.md §3):
       `git add -A && git commit -m "<type>(<scope>): <task summary>" -m "Refs: TODO #X.Y"`
  10. **PUSH ngay lên GitHub:** `git push origin main`.
       Nếu push fail → retry 1 lần sau 10s, vẫn fail → DỪNG báo user.
  11. Quay lại bước 1 cho tới khi TODO.md hết task chưa tick HOẶC gặp blocker.
```
...
- `git push origin main` fail (auth, network, conflict).
...
8. `docs/GIT_WORKFLOW.md` — bắt buộc trước commit (commit message format + push policy)
9. `docs/TECH_STACK.md` — khi cần biết dùng lib nào
10. `docs/WORKFLOW.md` — khi cần hiểu flow end-to-end
11. `docs/DECISIONS.md` — khi muốn hiểu "tại sao chọn cách này"
12. `docs/GLOSSARY.md` — khi gặp từ viết tắt / domain term không rõ
